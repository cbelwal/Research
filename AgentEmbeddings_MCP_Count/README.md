...
A machine learning research system that generates low-dimensional **agent embeddings** from user interaction data with **MCP (Model Context Protocol)** tools. The system models user behavior patterns through their tool usage and creates compact vector representations (embeddings) that enable user similarity analysis, clustering, and recommendation tasks.
...
- **Encoder output:** The agent embedding, shared in one coordinate system across users
...
`user_tool_matrix ≈ agent_embeddings × tool_factors`
