...
def run_algorithms_on_synthetic_data(All_C_hat_a:dict,algID:int):
    ...
    if(algID == 2):
        ...
        (MAT_E, loss_for_each_agent) = Alg_3_MatrixFactorization(
                embeddingDimensions=embeddingDimensions,
                testData=testData)
    else:
...
if __name__== "__main__":
    ...
    All_C_hat_a = Algorithm_1_DataPreparation()

    algorithmsIds = [2, 3]
    for algID in algorithmsIds:
        print(f"Computing embeddings for Algorithm {algID}...")
        (MAT_E, loss_for_each_agent) = \
            run_algorithms_on_synthetic_data(All_C_hat_a,algID)
        print(f"Storing embeddings for Algorithm {algID}...")
        store_results_in_file(MAT_E, loss_for_each_agent, algID=algID)
