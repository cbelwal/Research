# Given an agent ID and K, find the K closest agents from each algorithm
import os,sys
# ----------------------------------------------
...
class CTopKAgents:
    ...
    def get_non_canary_agent_ids(self):
        """Return all agent IDs that are not part of canary 1 or canary 2 groups."""
        canary_agent_set = set()
        for canary_id in self.canary_agents:
            canary_agent_set.update(self.canary_agents[canary_id])
        non_canary_ids = [uid for uid in self.all_agent_ids if uid not in canary_agent_set]
    ...
    def _compute_top_k_for_all_algorithms(self, agent_id, k):
        """
        Compute top K closest agents for the given agent_id across all algorithms.
        ...
        for alg_id in self.ALGORITHM_IDS:
            ...
            target_embedding = MAT_E[agent_id - 1]
    ...
    def find_top_k_closest_agents(self, agent_id, k):
        """
        For the given agent_id, find the K closest agents from each algorithm.
        Top K is computed separately by cosine distance and euclidean distance.
        """
        if agent_id not in self.all_agent_ids:
            print(f"Error: Agent ID {agent_id} not found in database.")
            return None

        results = self._compute_top_k_for_all_algorithms(agent_id, k)

        print("\n" + "=" * 70)
        print(f"TOP {k} CLOSEST AGENTS TO AGENT {agent_id}")
        print("=" * 70)
        ...
        for alg_id in self.ALGORITHM_IDS:
            ...
            for uid, dist in top_k_cosine:
                print(f"    Agent {uid:5d} -> {self.format_distance(dist)}")
    ...
    def find_common_top_k_between_alg_2_and_3(self, agent_id, k):
        """
        Find and print common top K agents between algorithms 2 and 3,
        ...
        print("\n" + "=" * 70)
        print(f"COMMON TOP {k} AGENTS BETWEEN ALGORITHM 2 AND 3 (Agent {agent_id})")
        print("=" * 70)
    ...
    def find_common_top_k_between_all_algorithms(self, agent_id, k):
        """
        Find and print common top K agents across all algorithms,
        separately for cosine and euclidean distances.
        """
        if agent_id not in self.all_agent_ids:
            print(f"Error: Agent ID {agent_id} not found in database.")
        ...
        print("\n" + "=" * 70)
        print(f"COMMON TOP {k} AGENTS ACROSS ALL ALGORITHMS (Agent {agent_id})")
...
if __name__ == "__main__":
    ...
    # Get a non-canary agent to test with
    non_canary_ids = topK.get_non_canary_agent_ids()
    print(f"Total non-canary agents: {len(non_canary_ids)}")
    print(f"Sample non-canary agent IDs: {non_canary_ids[:10]}")

    # Find top 5 closest agents for the first non-canary agent
    test_agent_id = non_canary_ids[0]
    topK.find_top_k_closest_agents(agent_id=test_agent_id, k=5)

    # Find common top K between algorithm 2 and 3
    topK.find_common_top_k_between_alg_2_and_3(agent_id=test_agent_id, k=5)

    # Find common top K across all 4 algorithms
    topK.find_common_top_k_between_all_algorithms(agent_id=test_agent_id, k=5)
